import { 
  users, menuItems, staff, tables, orders, orderItems,
...
import { db } from "./db";
import { eq, and, gte, lte, desc, sql, asc, isNull, not } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async getMenuItemsByCategory(category: string): Promise<MenuItem[]> {
    return db.select().from(menuItems).where(eq(menuItems.category, category)).orderBy(asc(menuItems.name));
  }
  ...
  async deleteMenuItem(id: number): Promise<boolean> {
    ...
    return true; // In PostgreSQL, delete doesn't return count
  }
  ...
  async getSalesByCategory(): Promise<{ category: string, totalSales: number }[]> {
    const result = await db
      .select({
        ...
        totalSales: sql`COALESCE(SUM(${orderItems.price} * ${orderItems.quantity}), 0)`,
      })
      .from(orderItems)
  ...
  async getPopularItems(limit = 5): Promise<{ menuItemId: number, menuItemName: string, orderCount: number }[]> {
    const result = await db
      ...
      .innerJoin(menuItems, eq(orderItems.menuItemId, menuItems.id))
      .innerJoin(orders, eq(orderItems.orderId, orders.id))
      .where(not(isNull(orderItems.menuItemId)))
